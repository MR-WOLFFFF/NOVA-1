function RPR3D(th1,rho,th2)
%% A function named RPR3D used to generate the DH model of the RPR robot in 3D space %%
...
%% Link twist and creation of prismatic joint (green) %%
    ...
    % Generation of prismatic joint (green) %

    rects(B,C,rz1*rx1,[0.47,0.67,0.19]); % Calling sub-function 'rects' to generate the prismatic joint %

    
    % Plot of link C-D %

    plot3([D(1),C(1)],[D(2),C(2)],[D(3),C(3)],'color',[0.47,0.67,0.19],'LineWidth',2)

    % Generation of second revolute joint (red) using function 'cylinder'%
    
    cylinderplot (theta,D,E,r,rz1*rx1,'r'); % Calling sub-function cylinderplot to create cylinder %

%% Generation of final link & end-effector (red)%

    % Sub-function rotations called to apply rotation/twist on corresponding axis%

    [~,ry1,~]= rotations(-pi/2);
    [~,~,rz2]= rotations(th2);
    Erot= rz1*rx1*rz2*ry1*[0;0;0];
    F= rz1*rx1*rz2*ry1*[0;0;25];
    E= E+Erot;
    F= E+F;

    % Plot of link E-F & end-effector %

    plot3([F(1),E(1)],[F(2),E(2)],[F(3),E(3)],'color','r','LineWidth',2)
    scatter3(F(1),F(2),F(3),'MarkerEdgeColor','k','MarkerFaceColor','r','Linewidth',1)
